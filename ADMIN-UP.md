...
### 9.4. Быстрая inline-правка «мини-карандаши» (мобильный сценарий)
Долгое удержание (0.5с, только палец/стилус — мышь игнорируется) плавающей кнопки-карандаша
включает режим: у редактируемых надписей появляются мини-карандаши, текст правится прямо на
странице (contentEditable `plaintext-only`). Выход — повторное удержание 0.5с.

- **Файлы:** `inline-edit.tsx` (контекст `InlineEditProvider`, `useInlineEdit`, `EditableText`,
  чипы, `InlineAdd`) и `editable-sections.tsx` (клиентские секции страницы: 4 блока, программа,
  что входит/не входит, что взять, заголовок/описание hero, цена «от»).
- **Что правится:** заголовок, описание, заголовки 4 блоков, время/заголовок/текст точек
  программы, тексты пунктов трёх списков, взрослая цена. Плюс добавление/удаление пунктов
  (удаление — с confirm). Фото/иконки — только через панель.
- **Сохранение:** те же `useAutosave`-payload'ы и PATCH-роут, что у панели (полная замена
  коллекций); статус показывается плашкой над кнопкой-карандашом.
- **КРИТИЧНО про дизайн:** в режиме просмотра Editable-компоненты рендерят разметку и классы
  1-в-1 как прежняя серверная вёрстка — при правке этих секций в `page.tsx` менять их надо
  СИНХРОННО в `editable-sections.tsx`. Мини-карандаши позиционируются абсолютно (ноль сдвигов
  макета); блоки растут с текстом естественно, как от более длинного текста из БД.
- Провайдер оборачивает всю страницу тура; посетителям без прав передаётся `tour={null}` —
  «пустой» контекст, никакого редактирования и лишних данных в HTML.
- При выключении режима — `router.refresh()`; свежий серверный объект подхватывается только
  вне режима и не во время сохранения (чтобы не перетереть правку).
...
src/components/admin/tour-editor/tour-edit-launcher.tsx  # редактор: кнопка/меню/разделы + long-press
src/components/admin/tour-editor/inline-edit.tsx      # контекст быстрой правки, EditableText, чипы
src/components/admin/tour-editor/editable-sections.tsx # клиентские секции страницы (вёрстка 1-в-1)
```
...
12. **Секции страницы тура живут в `editable-sections.tsx`.** Блоки/программа/списки/цена «от»
    и заголовок/описание hero рендерятся клиентскими компонентами (для inline-правки). Правишь
    вёрстку этих секций — правь ИХ, а не `page.tsx`. Разметка в режиме просмотра обязана
    оставаться 1-в-1 (см. 9.4). `contentEditable="plaintext-only"` не работает в старых Firefox
    (<136) — мобильный сценарий (Chrome/Safari) покрыт.
